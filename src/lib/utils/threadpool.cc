        ...
        m_Threads.emplace_back([=, this] {
            while (true) {
                Task task;
                {
                    ...
                    // As in the previous example, you can achieve the same variable-capture goals by using different combinations of
                    // capture options. The following list includes different capture options that produce the same result as [divisor,
                    // &sum] in the previous example.
                    // [=, &sum]: Captures any referenced variable within the lambda by value (making a copy), except sum that has to be
