...
FileManager::~FileManager() { filehelper = nullptr; }
...
void FileManager::ReleaseLock(const std::string& key) {
    ...
    if (target != fileLockMap.end()) {
        target->second = false;
        if (filehelper->m_bShouldLogFileWriting) stdlog("ReleaseLock:: " << key << " lock released to false");
    } else {
        // should never happen
        if (filehelper->m_bShouldLogFileWriting) stdlog("ReleaseLock:: " << key << " lock doesn't exist");
    }
    ProcessQueue();
}

void FileManager::QueueFileWrite(const std::string path, const std::string file, const std::string data, const FileWriteOptions options) {
    if (filehelper->m_bShouldLogFileWriting)
        stdlog("FileManager::QueueFileWrite() before locking | path: " << path << " file: " << file << " data: " << data);
    queueMutex.lock();
    if (filehelper->m_bShouldLogFileWriting) stdlog("FileManager::QueueFileWrite() after locks");
...
void FileManager::ProcessQueue() {
    if (queueMutex.try_lock()) {
        if (filehelper->m_bShouldLogFileWriting) stdlog("FileManager::ProcessQueue() start");
        for (auto itr = queuedFileWrites.begin(); itr != queuedFileWrites.end(); ++itr) {
            ...
            if (current != fileLockMap.end() && !current->second) {
                ...
                if (target->data.size() > 0 && (target->file.size() > 0 || target->path.size() > 0) && !target->processed) {
                    ...
                    target->processed = 1;
                    filehelper->TryWriteFile(target->path, target->file, target->data, target->options, true);
                }
            }
        }
        queueMutex.unlock();
        if (filehelper->m_bShouldLogFileWriting) stdlog("FileManager::ProcessQueue() finished unlocking");
    } else {
        if (filehelper->m_bShouldLogFileWriting) stdlog("FileManager::ProcessQueue() locked cant start");
    }
}
